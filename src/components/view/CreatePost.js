...
//		console.log('subimtPost: '+JSON.stringify(this.state.post))
	...
	imageSelected(files){
		...
		const cloudName = 'docrp0ved'
		const url = 'https://api.cloudinary.com/v1_1/'+cloudName+'/image/upload'

		const timestamp = Date.now()/1000
		const uploadPreset = 'pzhqz8er'

		const paramsStr = 'timestamp='+timestamp+'&upload_preset='+uploadPreset+'5lFzejyzEGBjjlLX0yGvbu9cFqw'

		const signature = sha1(paramsStr)
		const params = {
			'api_key': '787695514738342',
			'timestamp': timestamp,
